...
class _HalamanEditProfileState extends State<HalamanEditProfile> {
  ...
  Future<void> _loadNamaPengguna() async {
    final response = await http.post(
      Uri.parse(
          'http://10.0.2.2/mY_Warung/Web/pembeli/update_user_profile.php'),
      body: {
        'action': 'get',
  ...
  void validasiNama() {
    if (namaController.text.isEmpty) {
      setState(() => namaError = "Nama tidak boleh kosong");
    } else {
  ...
  void validasiUsername() {
    if (usernameController.text.isEmpty) {
      setState(() => usernameError = "Username tidak boleh kosong");
    } else {
  ...
  void validasiPassword() {
    if (passwordController.text.isEmpty) {
      ...
      setState(() => passwordError = "Password harus minimal 8 karakter");
    } else if (!RegExp(r"[A-Z]").hasMatch(passwordController.text)) {
      setState(() => passwordError = "Password harus mengandung huruf besar");
    } else if (!RegExp(r"[a-z]").hasMatch(passwordController.text)) {
      setState(() => passwordError = "Password harus mengandung huruf kecil");
    } else if (!RegExp(r"[0-9]").hasMatch(passwordController.text)) {
      setState(() => passwordError = "Password harus mengandung angka");
    } else {
  ...
  void validasiKonfirmasiPassword() {
    ...
    } else if (konfirmasiPasswordController.text != passwordController.text) {
      setState(() => konfirmasiPasswordError = "Password tidak sama");
    } else {
      setState(() => konfirmasiPasswordError = null);
    }
  }
    ...
    if (namaError == null &&
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'success') {
          ...
          setState(() => usernameError = data['message']);
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
  ...
  Future<List<Map<String, dynamic>>> _fetchPurchaseHistory() async {
    final response = await http.get(Uri.parse(
        'http://10.0.2.2/mY_Warung/Web/pembeli/get_purchase_history.php?username=${widget.currentUsername}'));
    if (response.statusCode == 200) {
  ...
  void toggleShowKonfirmasiPassword() =>
      setState(() => showKonfirmasiPassword = !showKonfirmasiPassword);
